...
describe('elevenlabs link configuration', () => {
  it('uses trusted booking hosts by default', () => {
    expect(
      getPublicElevenLabsMarkdownLinkAllowedHosts({} as NodeJS.ProcessEnv),
    ).toBe('calendar.notion.so,notion.so,cal.com,design-prism.com')
  })
  ...
  it('only allows trusted https destinations', () => {
    ...
    expect(
      isAllowedElevenLabsExternalUrl(
        'https://calendar.notion.so/demo',
        allowedHosts,
      ),
    ).toBe(true)
    expect(
      isAllowedElevenLabsExternalUrl(
        'https://www.cal.com/prism/demo',
        allowedHosts,
      ),
    ).toBe(true)
    expect(
      isAllowedElevenLabsExternalUrl(
        'https://design-prism.com/get-started',
        allowedHosts,
      ),
    ).toBe(true)
    expect(
      isAllowedElevenLabsExternalUrl(
        'http://calendar.notion.so/demo',
        allowedHosts,
      ),
    ).toBe(false)
    expect(
      isAllowedElevenLabsExternalUrl('https://evil.example/demo', allowedHosts),
    ).toBe(false)
    expect(isAllowedElevenLabsExternalUrl('not a url', allowedHosts)).toBe(
      false,
    )
  })
  ...
  it('registers a guarded external redirect client tool for the widget', () => {
    ...
    const cleanup = registerElevenLabsClientTools(
      widget,
      'calendar.notion.so,cal.com',
    )
    const detail: {
      config?: {
    ...
    const redirectToExternalURL =
      detail.config?.clientTools?.redirectToExternalURL
