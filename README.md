...
- **Get-started assistant surface** – `/get-started` relies on the stock ElevenLabs floating widget that mounts through `components/global-elevenlabs-widget.tsx`; `/apply` intentionally suppresses it so the active application stays focused. The old custom `SalesChat` client is no longer mounted on the live site.
