...
- **Get-started assistant surface** – `/get-started` and `/apply` rely on the stock ElevenLabs floating widget that mounts through `components/global-elevenlabs-widget.tsx`; the old custom `SalesChat` client is no longer mounted on the live site.

### Assistant architecture (at a glance)

- `app/get-started/page.tsx` is now the overview/qualification entry page for the Prism application flow, and `app/apply/page.tsx` is the dedicated 2-step application route.
...
- `components/runtime-client-shell.tsx` now keeps route-surface setup on the critical path and defers analytics, monitors, Vercel Analytics, toaster wiring, and the stock ElevenLabs embed/widget into `components/runtime-deferred-features.tsx` during browser idle time.
- `components/global-elevenlabs-widget.tsx` is now the single public widget surface. It mounts on public inner pages but stays off the homepage, defaults to collapsed when there is no saved user preference, and persists later manual expand/collapse choices in browser storage.
...
- Public widget invariant: the host should stay fixed with a top-most z-index so nav, skip links, charts, and other sticky/fixed page chrome never render above the visible widget surface.
...
- Expected runtime invariants:
  - On `/` and inner pages, the widget host should resolve to `position: fixed` with the elevated global z-index and remain the topmost element in the widget’s visible region after scrolling.
