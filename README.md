...
- **Architecture** – Marketing forms post to Formspree with HTML5/client-side validation and `fetch`. Most use the shared `useFormValidation` hook; the one-question-at-a-time Website Intake owns equivalent step validation, request locking, and its in-page success flow. We do **not** use React Hook Form, Zod, or server actions for these flows today. If this changes, update this section immediately.
- **Waitlist funnel (2026-09-14, supersedes booking/intake CTAs below)** – Prism is at capacity. Every public sales CTA and every lead form is the waitlist: `lib/waitlist.ts` owns `WAITLIST_CTA` (label "Join the waitlist", href `/waitlist`), `WAITLIST_FOCUS_HREFS` (`/waitlist?focus=website|content|ads` for service pages, homepage offer cards, and the `/ig` `/tiktok` `/youtube` hubs), the shared capacity copy rendered by `components/waitlist/CapacityNotice.tsx`, and the Formspree endpoint (`NEXT_PUBLIC_WAITLIST_FORM_ENDPOINT`, falling back to the Contact form `xjkjbpdb` until the dedicated form exists). `components/forms/WaitlistForm.tsx` (Formspree + `useFormValidation` + fetch → `/waitlist/thank-you`, noindex, `LeadSuccessTracker`) is the only live sales form. `/website-intake`, `/content-intake`, `/ads-intake`, `/get-started`, `/apply`, `/free-analysis`, `/contact`, `/aeo`, `/book-a-shoot`, and `/ai` 308-redirect to `/waitlist`; their page and form code is deleted. `BOOK_A_CALL_CTA` and `WEBSITE_START_CTA` no longer exist; `lib/booking.ts` `BOOKING_URL` survives only for the invite-gated `/chatgpt-ads` partner unlock and the legacy post-payment `/thanks` page. `FREE_AUDIT_CTA_TEXT` now reads "Join the waitlist". The floating ElevenLabs launcher mounts only on non-mobile `/pricing`. `/refer`, `/scholarship`, `/models`, and design votes are not sales forms and keep working. `lib/pricing-consistency.ts` forbids the retired booking/intake tokens on pricing-sensitive surfaces and `__tests__/visual/locked-routes.spec.ts` locks `/waitlist` in place of `/get-started`. Where older bullets in this file describe booking a 30-minute call or the intake routes as the primary action, treat them as historical context.
...
- **Canonical pricing policy (all-call-first, 2026-07-27)** – `lib/pricing-model.ts` is the single source of truth and models Prism's four packaged offers on `/pricing`: **Website** (the ultra-premium "PRO website" at `/websites`), **Content OS** (`/content`), **Dental OS** (`/dental-os`), and **Prism Infinity** (`/prism-infinity`). **NO offer shows public exact pricing** — every offer is scoped on a **30-minute Zoom call** booked through `BOOK_A_CALL_CTA` (href = `BOOKING_URL` from `lib/booking.ts`, new tab + `noopener noreferrer`). Website-start surfaces use `WEBSITE_START_CTA` ("Start my website" → `/website-intake`); `/pricing` cards stay `BOOK_A_CALL_CTA`. Never reintroduce public dollar amounts: the retired `$300`, `$100/month`, `$5,000`, `$1,000/month`, and `$2,000/month` tokens are forbidden on pricing-sensitive surfaces, and all `Offer` structured data stays price-free. `/pricing` compares all four. Self-serve checkout is retired (`lib/payment-links.ts`, `WebsiteOrderForm`, `MobileOrderBar`, and `BaseOfferShowcase` were deleted; the noindex `/checkout/website/thank-you` route stays as the landing target for the legacy live Stripe link). The old five-tier ladder is retired — **except** `/get-started`, kept as a free on-ramp (free Growth Dashboard + request a free deep audit) surfaced in footer and a homepage callout. Always spell `/month` (never `/mo`). `pnpm verify:pricing-consistency` gates deploys. The retired `/founder-os` route now 301-redirects to `/content`; other legacy pricing routes still redirect to `/pricing`. `/ads` ships a price-free offer schema pointing at `/ads`; `/seo`, `/local-listings`, `/dental-website`, and `/dental-practice-seo-expert` ship price-free offer schemas pointing at `/pricing`. `pricing-schema-consistency.test.ts` blocks retired pricing from reappearing.
...
- **Deploy mode policy** – GitHub Actions is the only production publisher. `main` source deploys through `.github/workflows/deploy.yml`, while Vercel Git auto-deploy for `main` is disabled in `vercel.json` to avoid duplicate production releases. The locked-route screenshot job runs in CI and blocks the deploy job. Because the dark theme lets large copy changes hide inside the 5% pixel tolerance (a full pricing redesign moves only ~4% of pixels), the locked spec also asserts rendered copy directly: locked routes must keep their call-first phrases and must never render retired public pricing tokens.
...
- **Assistant surface** – Prism has two supported public ElevenLabs surfaces: the floating launcher on non-mobile `/pricing` and `/contact`, plus the consent-gated inline homepage Prism Guide after `HomeFitSection`. The homepage surface is default-off in code, explicitly enabled in production, and loads vendor code only after near-viewport activation, desktop/WebGL 2 checks, and affirmative consent. Focused flows such as `/get-started` and `/apply` stay widget-free.

### Assistant and funnel architecture

- `app/waitlist/page.tsx` is the single live sales funnel (Prism is at capacity); `/get-started` and `/apply` 308-redirect there and their code is deleted.
...
- `app/websites/page.tsx` is the ultra-premium **PRO website** offer (2026-07-27 repositioning): design systems, software-grade engineering, and analytics wired from day one, structured to rank on Google and get cited by AI assistants (ChatGPT, Gemini, Claude, Perplexity). There is **no public price and no self-serve checkout**. Primary CTAs say "Start my website" and route to the focused `/website-intake` funnel; a secondary `BOOK_A_CALL_CTA` stays on the page. The page keeps the proof-build gallery, measured Search Console results, a three-pillar Design/Engineering/Analytics section, a Google + AI visibility section, and a call-first process. The old order dialog (`WebsiteOrderForm.tsx`), sticky `MobileOrderBar.tsx`, `BaseOfferShowcase.tsx`, and `lib/payment-links.ts` were deleted.
- `app/content/page.tsx` is the public **Content** service page (packaged as **Content OS** on `/pricing`): a system that plans, produces, and publishes across the website and every social platform, implemented over 3 months, then optimized monthly, and scoped on a 30-minute call. Ads is a sibling service at `/ads`, not the Content hero claim. It replaced the retired Founder OS — `/founder-os` and `/founder-os/apply` now 301-redirect to `/content` in `next.config.mjs`. `app/dental-os/page.tsx` (the full Prism growth system packaged for dental practices, custom-priced) and `app/prism-infinity/page.tsx` (unlimited services on one monthly subscription, scoped on a call) complete the four packaged offers on `/pricing`. Founder OS page and form files are gone; keep only the redirects.
...
- `components/global-elevenlabs-widget.tsx` owns the floating launcher on non-mobile `/pricing` and `/contact`; it defaults to collapsed when there is no saved user preference and persists later manual expand/collapse choices in browser storage.
- `components/home/HomeElevenLabsAgentSection.tsx` owns the separately gated inline homepage surface. It keeps the vendor script and custom element unmounted until the visitor accepts the AI/recording notice; mobile and unsupported-WebGL browsers receive the first-party `/get-started` fallback.
...
- Deploy workflow runs in order: `UI Lock Screenshots` (`pnpm test:visual:locked`, blocking) -> `Build and Deploy`.
...
- The `/websites` self-serve order form is retired. The PRO website flow now starts on `/website-intake` (four questions, then optional Notion Calendar booking on the success screen) and posts to the dedicated Prism **Website Intake** Formspree form (`xrpzlkrd`); see [`docs/forms.md`](./docs/forms.md#formspree-dashboard-configuration). The noindex `/checkout/website/thank-you` route remains only as the landing target for the legacy live Stripe Payment Link (its GA4 `purchase` tracker still fires if a straggler completes that link).
...
- Search metadata is generated through `lib/seo/rules.ts`: titles are capped at 48 characters and descriptions at 96 characters across static routes, blog posts, and library details. Run `pnpm seo:inventory` and `pnpm seo:lint` after metadata changes.
- Every route, including blog posts and library details, uses `public/prism-opengraph.png` for Open Graph and Twitter previews. Blog featured images remain available for cards and article heroes.
