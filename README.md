...
- **Canonical pricing policy** – `/pricing` is the canonical Growth Dashboard pricing URL: free Growth Dashboard, included Light Audit, normally `$500` Deep Growth Audit, 60-day Growth Sprints starting at `$3,500`, and optional ongoing Growth Partner support starting at `$1,500/month`. `/websites` is the dedicated exception for the selective one-time website build offer starting at `$300`; do not move that offer into the main pricing model. Legacy pricing routes permanently redirect to `/pricing`.
...
- `app/websites/page.tsx` is the selective one-time website build offer. `components/forms/WebsiteBuildEstimatorForm.tsx` powers its estimator, posts to `NEXT_PUBLIC_WEBSITE_BUILD_FORM_ENDPOINT` / `https://formspree.io/f/xpqebnbz`, and redirects accepted submissions to `/thank-you?source=website-build`.
- `app/founder-os/page.tsx` is the premium Founder OS offer (a managed AI command layer) — a distinct high-ticket track built in a self-contained **light Geist** design that is intentionally separate from the dark site (scoped via the `[data-surface='founder-os']` attribute in `app/globals.css`; see `DESIGN.md`). Its selective application lives at `app/founder-os/apply/page.tsx` via `components/forms/FounderOsApplicationForm.tsx`, which posts to `NEXT_PUBLIC_FOUNDER_OS_FORM_ENDPOINT` / `https://formspree.io/f/xkoalapv` and shows an in-place confirmation (no thank-you redirect). `/founder-os` is indexable; `/founder-os/apply` is noindex. The Founder OS pricing ($10k Blueprint → from $50k installation → from $10k/month managed) is intentional to this page only and does not change `/pricing`. The page uses a premium command-system design: animated command-orbit hero, command demo, bottleneck ledger, install registry, agent/runbook cards, control ladder, Blueprint packet, rollout timeline, logo wall, measurement console, and final command CTA. Founder OS logo SVGs live in `public/logos/founder-os/`, render via plain `<img>`, and are cached immutably via `next.config.mjs`. Founder OS surface copy is kept em-dash-free.
