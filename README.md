...
- `app/founder-os/page.tsx` is the premium Founder OS offer (a managed AI operating layer) — a distinct high-ticket track built in a self-contained **light Geist** design that is intentionally separate from the dark site (scoped via the `[data-surface='founder-os']` attribute in `app/globals.css`; see `DESIGN.md`). Its selective application lives at `app/founder-os/apply/page.tsx` via `components/forms/FounderOsApplicationForm.tsx`, which posts to `NEXT_PUBLIC_FOUNDER_OS_FORM_ENDPOINT` / `https://formspree.io/f/xkoalapv` and shows an in-place confirmation (no thank-you redirect). `/founder-os` is indexable; `/founder-os/apply` is noindex. The Founder OS pricing ($10k Blueprint → from $50k installation → from $10k/month managed) is intentional to this page only and does not change `/pricing`. The page also shows the real stack it connects to: an integrations logo wall plus a frontier-model-provider row, with brand SVGs sourced from `svgl.app` and stored in `public/logos/founder-os/` (rendered via plain `<img>`, grayscale to color on hover). Founder OS surface copy is kept em-dash-free.
