...
- `app/websites/page.tsx` is the ultra-premium **PRO website** offer (2026-07-27 repositioning): design systems, software-grade engineering, and analytics wired from day one, structured to rank on Google and get cited by AI assistants (ChatGPT, Gemini, Claude, Perplexity). There is **no order form, no price, and no self-serve checkout** — every CTA is `BOOK_A_CALL_CTA` (the 30-minute Notion Calendar Zoom link, new tab). The page keeps the proof-build gallery, measured Search Console results, a three-pillar Design/Engineering/Analytics section, a Google + AI visibility section, and a call-first process. The old order dialog (`WebsiteOrderForm.tsx`), sticky `MobileOrderBar.tsx`, `BaseOfferShowcase.tsx`, and `lib/payment-links.ts` were deleted.
...
- The `/websites` order form is retired: the PRO website flow is booking-only via the Notion Calendar link. The noindex `/checkout/website/thank-you` route remains only as the landing target for the legacy live Stripe Payment Link (its GA4 `purchase` tracker still fires if a straggler completes that link).
