...
- `app/websites/page.tsx` is the `$300` flat one-time website offer. The buyer opens a fullscreen one-question-at-a-time order dialog (launcher panel on the page, plus a `#order` deep link used by the hero CTA and the sticky mobile order bar), answers six steps with a live order-manifest rail, reviews, submits (Formspree at `NEXT_PUBLIC_WEBSITE_BUILD_FORM_ENDPOINT` / `https://formspree.io/f/xpqebnbz`; drafts persist in same-tab `sessionStorage` under `prism_website_order_draft_v1`), then a staged in-dialog success state opens the `$300` Stripe Payment Link in a new tab to kick off the build. Delivered in ~7 days with infinite iterations; the finished site is 100% theirs, with an optional `$100/month` care plan afterward. `components/forms/WebsiteOrderForm.tsx` powers the launcher → fullscreen dialog → staged success → pay flow (it replaces the retired `WebsiteBuildEstimatorForm.tsx` dynamic estimator). The page no longer collects no card / reviews-before-payment; that selective model is retired.
