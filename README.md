...
- The `/websites` self-serve order form is retired. The PRO website flow now starts on `/website-intake` (four questions, then optional Notion Calendar booking on the success screen). Formspree still needs a dedicated **Website Intake** form from someone with dashboard access; see [`docs/forms.md`](./docs/forms.md#formspree-dashboard-handoff-needs-account-access). The noindex `/checkout/website/thank-you` route remains only as the landing target for the legacy live Stripe Payment Link (its GA4 `purchase` tracker still fires if a straggler completes that link).
