...
- Header nav labels live in `NAV_ITEMS` in `lib/constants.ts`; the current public nav (2026-07-27 redesign) is a flat seven-item rail: the four offers, then `wall of love` + `case studies`, then `contact` — no dropdown, no `pricing` or `get started` items, and NO CTA button. Desktop links are quiet rounded pills (active `white/[0.08]` wash) in three hairline-divided groups (offers | proof | contact), taking over from the menu button at `lg` (1024px); the logo tagline shows only from `xl`. The fixed mobile sheet is pinned between the measured chrome and the visual-viewport bottom (same three groups, no index prefixes), stays out of flow so `--prism-header-height` does not jump, locks scroll on both `html` and `body`, marks only content outside the header `inert` while preserving prior state, and animates in with staggered links. `pnpm test:mobile-navbar` covers the interaction in mobile Chromium and WebKit. The footer System column links `PRO Website` and `Content OS` (the old `Founder OS` link became Content OS), and the Company column carries `Refer a friend — $100` (`/refer`).
...
- The homepage hero is built from `components/home/HomeHeroSection.tsx`; copy comes from `components/home/homepage-content.ts`. The H1 is "Prism" with the subhead "the #1 growth partner for small businesses". Its proof block reads from `lib/proof-metrics.ts`, the canonical snapshot for cross-site proof. As verified August 21, 2026, it shows 16,882 new users across 17 connected client sites in July 2026, a 74K combined audience, and 1.2M TikTok views in the latest 60 days. Channel details are 24.7K YouTube subscribers / 5.8M views, 38K Instagram followers / 644 posts, and 11.3K TikTok followers / 1.2M views in 60 days. Traffic uses GA4 production hostnames only; social data uses public profiles plus authenticated TikTok analytics. Do not replace this with revenue claims or sum unrelated lifetime views.
