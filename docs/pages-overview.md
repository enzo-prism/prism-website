...
- `/ig`, `/tiktok`, and `/youtube` are link-in-bio hubs with no global floating assistant. Their shared page (`components/social-link-hub.tsx`) is a single dark, mobile-first column with deliberately minimal copy: a platform-aware "The studio behind the videos / feed / channel" headline, a two-number proof strip (no supporting paragraph), then exactly two action cards — Premium Website Design (`/websites`, primary, detail "rank on ChatGPT and Google") and Refer a friend (`/refer`, detail "You get $100 when they become a client"). Typography (2026-07-29): everything that reads as language — the profile handle, proof strip, and card detail lines — is Geist Sans mixed-case at body sizes (14–16px, `#b8afa2`/`#cfc7ba` on dark, `black/60` on the light primary card); tiny uppercase Geist Mono survives only as the header "prism" wordmark and the footer "© prism". Do not reintroduce 10px uppercase mono for sentence copy on these pages. The 2026-07-29 simplification cut the other five cards (case studies, Content OS, Prism Infinity, Start free, and the platform cross-link); do not reintroduce them. The Marble App Store card was removed from the client funnel. Keep copy minimal; do not reintroduce ranked credit lists, product screenshot showcases, gratitude framing, or extra sections.
