...
- `/ig`, `/tiktok`, and `/youtube` are link-in-bio hubs with no global floating assistant. Their shared page (`components/social-link-hub.tsx`) is a single dark, mobile-first column with deliberately minimal copy: the shared headline "Grow your business with Prism", one supporting line ("We implement the strategies and tactics we post about to level up your business."), a two-number platform proof strip, then exactly two action cards segmented by founder revenue via short sans routing questions rendered ABOVE the cards ("Doing under $1M a year?" → Premium Website Design, `WEBSITE_START_CTA` → `/website-intake`, primary, detail "rank on ChatGPT and Google"; "Doing $1M–$10M a year?" → Prism Infinity, `/prism-infinity`, quiet, detail "unlimited landing pages, ads, and websites" — never the retired "Everything Prism, unlimited" phrase and never a price). Referral is intentionally absent from these focused routes. The questions are aria-hidden; each offer card carries the segmentation in its aria-label instead, so assistive tech hears it once on the actionable element. Typography: everything that reads as language — the profile handle, supporting line, proof strip, and card detail lines — is Geist Sans mixed-case at body sizes (14–16px, `#b8afa2`/`#cfc7ba` on dark, `black/60` on the light primary card); tiny uppercase Geist Mono survives only as the header "prism" wordmark and the footer "© prism". Do not reintroduce 10px uppercase mono for sentence copy on these pages. Keep the previously retired cards (case studies, Content OS, Start free, platform cross-links, referral, and Marble) out of the social funnel. Keep copy minimal; do not reintroduce ranked credit lists, product screenshot showcases, gratitude framing, or extra sections.
