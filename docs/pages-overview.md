...
- All non-homepage pages mount the stock floating ElevenLabs launcher via `components/global-elevenlabs-widget.tsx`, initialized from `components/runtime-client-shell.tsx`. It only hides on `/get-started` when that route's dedicated `SalesChat` launcher is actually visible.
