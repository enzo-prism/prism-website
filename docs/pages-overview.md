...
- `/ig`, `/tiktok`, and `/youtube` are link-in-bio hubs with no global floating assistant. Their shared page (`components/social-link-hub.tsx`) is a single dark, mobile-first column with deliberately minimal copy (2026-07-30): the shared headline "Grow your business with Prism", one supporting line ("We implement the strategies and tactics we post about to level up your business."), a two-number proof strip (platform attention stat · "$5 million+ revenue driven for clients"), then exactly three action cards segmented by founder revenue via short sans routing questions rendered ABOVE the two offer cards ("Doing under $1M a year?" → Premium Website Design, `/website-intake`, primary, detail "rank on ChatGPT and Google"); "Doing $1M–$10M a year?" → Prism Infinity, `/prism-infinity`, detail "unlimited landing pages, ads, and websites" — never the retired "Everything Prism, unlimited" phrase and never a price), and Refer a friend (`/refer`, detail "You get $100 when they become a client") after a hairline divider so it never reads as a third revenue tier. The questions are aria-hidden; each offer card carries the segmentation in its aria-label instead, so assistive tech hears it once on the actionable element. Typography (2026-07-29): everything that reads as language — the profile handle, supporting line, proof strip, and card detail lines — is Geist Sans mixed-case at body sizes (14–16px, `#b8afa2`/`#cfc7ba` on dark, `black/60` on the light primary card); tiny uppercase Geist Mono survives only as the header "prism" wordmark and the footer "© prism". Do not reintroduce 10px uppercase mono for sentence copy on these pages. The 2026-07-29 simplification cut the other retired cards (case studies, Content OS, Start free, and the platform cross-link); do not reintroduce those. The Marble App Store card was removed from the client funnel. Keep copy minimal; do not reintroduce ranked credit lists, product screenshot showcases, gratitude framing, or extra sections.
...
- The `$100` referral program page (noindex, sitemap-excluded; `/referral`, `/referrals`, and `/affiliate` all redirect here). Dark system, one column: "A friend who becomes a client. $100." hero, a 3-step how-it-works, the on-page `components/forms/ReferralForm.tsx` (Formspree `NEXT_PUBLIC_REFERRAL_FORM_ENDPOINT` → `meebpgaj`, required permission-to-share consent, in-page success with a "Refer another friend" reset), a payout FAQ, and fine print ($100 is a referral payout, not service pricing; paid when the referred business becomes a paying client — website, Content OS, Dental OS, or Prism Infinity; self-referrals excluded; no cap).
...
- Deliberately minimal (2026-07-29 redesign): a dark core-route hero ("Talk to Prism." + "Send a note. We reply within 24 hours."), the `ContactForm` card, and one "Prefer email?" line with `support@design-prism.com`. The old `What to Expect` checklist and Silicon Valley blurb stay retired; do not reintroduce them.
...
- Header nav labels live in `NAV_ITEMS` in `lib/constants.ts`; the current public nav (2026-07-27 redesign) is a flat seven-item rail: the four offers, then `wall of love` + `case studies`, then `contact` — no dropdown, no `pricing` or `get started` items, and NO CTA button. Desktop links are quiet rounded pills (active `white/[0.08]` wash) in three hairline-divided groups (offers | proof | contact), taking over from the menu button at `lg` (1024px); the logo tagline shows only from `xl`. The mobile sheet is pinned under the 72px chrome (same three groups, no index prefixes), stays out of flow so `--prism-header-height` does not jump, locks scroll on both `html` and `body`, marks the page behind it `inert`, and animates in with staggered links. The footer System column links `PRO Website` and `Content OS` (the old `Founder OS` link became Content OS), and the Company column carries `Refer a friend — $100` (`/refer`).
...
- The homepage deck features **current-client websites only**. Former engagements stay on `/case-studies` as archive pages but must not appear in `HOMEPAGE_CLIENT_WINS`, offer-page proof, pricing proof, dental rosters, or `llms.txt` measured examples. The exclusion list lives in `lib/former-clients.ts` (currently Saorsa Growth Partners, sr4 Partners, and Laguna Beach Dental Arts) and is guarded by `__tests__/former-clients-highlight.test.ts`. Slide order is hand-curated to lead with measured Search Console wins (Olympic Bootworks, Belize Kids, Dr. Wong, Roseville). Do not invent metrics. Each slide carries `company` (the brand/business name shown as the prominent card label — **never a person's name**), `location`, `contextLabel`, plus `href` + `image` derived from the slug.
...
- Social proof copy in the hero is intentionally hard-coded as: `Instagram: 37,000+`, `TikTok: 10,000+`, `YouTube: 24,000+` (refreshed 2026-07-30).
