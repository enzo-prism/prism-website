...
- `/ig`, `/tiktok`, and `/youtube` are link-in-bio hubs with no global floating assistant. Their shared page (`components/social-link-hub.tsx`) is a single dark, mobile-first column with deliberately minimal copy: the shared headline "Grow your business with Prism", one supporting line ("We implement the strategies and tactics we post about to level up your business."), then exactly two action cards segmented by founder revenue via short sans routing questions rendered ABOVE the cards ("Doing under $1M a year?" → Premium Website Design, `WEBSITE_START_CTA` → `/website-intake`, primary, detail "rank on ChatGPT and Google"; "Doing $1M–$10M a year?" → Prism Infinity, `/prism-infinity`, quiet, detail "unlimited landing pages, ads, and websites" — never the retired "Everything Prism, unlimited" phrase and never a price). Keep the area between the supporting line and actions empty: follower, subscriber, view, post, reach, and revenue stats are intentionally absent from these focused routes. Referral is also intentionally absent. The questions are aria-hidden; each offer card carries the segmentation in its aria-label instead, so assistive tech hears it once on the actionable element. Typography: everything that reads as language — the profile handle, supporting line, and card detail lines — is Geist Sans mixed-case at body sizes (14–16px, `#b8afa2`/`#cfc7ba` on dark, `black/60` on the light primary card); tiny uppercase Geist Mono survives only as the header "prism" wordmark and the footer "© prism". Do not reintroduce 10px uppercase mono for sentence copy on these pages. Keep the previously retired cards (case studies, Content OS, Start free, platform cross-links, referral, and Marble) out of the social funnel. Keep copy minimal; do not reintroduce proof strips, ranked credit lists, product screenshot showcases, gratitude framing, or extra sections.
...
- The retired program ("earn up to $1,000" + external Typeform) must not come back; active entry points are the footer "Refer a friend — $100" link, the `/ads` + `/local-listings` referral blurbs, and the legacy referral redirects. Do not add referral back to the focused social link hubs.
