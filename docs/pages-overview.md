...
- The shared floating ElevenLabs launcher mounts only on non-mobile `/pricing` and `/contact` via `components/global-elevenlabs-widget.tsx` in `components/runtime-client-shell.tsx`. The homepage separately places one bounded Prism guide after `HomeFitSection`, before `HomeHowItWorksSection`, when `NEXT_PUBLIC_ELEVENLABS_HOMEPAGE_ENABLED=true`; it stays first-party until the visitor accepts the AI/recording notice and never loads the vendor runtime on mobile or without WebGL 2. Focused routes such as `/get-started` and `/apply` remain widget-free.
