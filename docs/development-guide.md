...
- Event-level attribution sent to GA4 should stay low-cardinality. Keep campaign context to `utm_source`, `utm_medium`, `utm_campaign`, `landing_path`, and first-touch source/medium/campaign; do not push raw click IDs (`gclid`, `fbclid`, `msclkid`), free-text queries, or other one-off identifiers into regular GA4 events.
...
- `/ig` and `/tiktok` now rely on `components/social-clip-landing-page.tsx` for shared CTA/outbound-link tracking. Keep the header profile link, action chips, source links, and footer contact link instrumented there so inbound social traffic keeps a full click trail.
