...
- `/ig`, `/tiktok`, and `/youtube` rely on `components/social-link-hub.tsx` for shared CTA/outbound-link tracking. Keep the header profile link on `trackExternalLinkClick`, and keep exactly two internal action cards on `trackCTAClick`: Premium Website Design (`WEBSITE_START_CTA` → `/website-intake`, primary) and Prism Infinity (`/prism-infinity`, quiet). Preserve the `{platform} landing header|actions` locations so inbound social traffic keeps a full click trail. Referral is intentionally absent from these focused link-in-bio routes. See `docs/pages-overview.md` for the hub's copy and typography rules (sans mixed-case body; mono-caps only for the wordmark and footer).
