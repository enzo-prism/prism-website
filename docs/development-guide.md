...
- Primary nav labels live in `lib/constants.ts` (`NAV_ITEMS`): a flat five-item list — the four offers (`OFFER_NAV_ITEMS`) plus `CONTACT_NAV_ITEM`, with no dropdown. The desktop rail renders the offer group, a hairline divider, then contact; the mobile sheet renders the same flat list with mono index prefixes. `/pricing` and `/get-started` are intentionally NOT in the top nav (they stay reachable via footer, homepage callout, and offer pages). When you add or remove an item, verify the desktop rail still fits at the `lg` (1024px) takeover width, the mobile sheet, locked route snapshots, and a representative case study detail page.
