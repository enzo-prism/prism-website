...
- `/ig`, `/tiktok`, and `/youtube` rely on `components/social-link-hub.tsx` for shared CTA/outbound-link tracking. Keep the header profile link on `trackExternalLinkClick`, and the three internal action cards (`/websites` primary, `/prism-infinity` and `/refer` quiet — the revenue-segmented set since 2026-07-30) on `trackCTAClick` with the `{platform} landing header|actions` locations (`prism infinity` is a new cta_text value at these locations as of 2026-07-30), so inbound social traffic keeps a full click trail. See `docs/pages-overview.md` for the hub's copy and typography rules (sans mixed-case body; mono-caps only for the wordmark and footer).
