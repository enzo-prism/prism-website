...
- `HomeHeroAgent` now uses the stock ElevenLabs widget path again: the official `https://unpkg.com/@elevenlabs/convai-widget-embed` script plus an inline `<elevenlabs-convai>` element with `variant="expanded"`.
- Treat the live production widget and ElevenLabs' official widget docs as the design source of truth. The homepage should feel like the official widget dropped into a well-staged Prism hero, not like a custom chat surface that happens to talk to ElevenLabs.
- Keep homepage customization wrapper-level only. Match the official widget design as closely as possible and avoid styling the widget internals or rebuilding the chrome in React unless there is a product-critical reason.
- The one intentional homepage-only exception is centering: `components/home/HomeHeroAgent.tsx` injects a tiny desktop-only Shadow DOM override so the expanded widget card sits in the middle of the hero stage instead of right-aligning against the viewport. Keep that override narrowly scoped to the homepage.
- The only public configuration we intentionally pass into the stock widget is the agent id and the markdown-link allowlist. If booking links should be clickable, the agent needs to return a real markdown link and the booking host must be present in `NEXT_PUBLIC_ELEVENLABS_MARKDOWN_LINK_ALLOWED_HOSTS`.
- Mobile spacing should stay simple: let the stock widget own its own layout and use `HomeHeroAgent` only for placement and width.
- The public agent id resolves via `lib/elevenlabs.ts` and can be overridden with `NEXT_PUBLIC_ELEVENLABS_AGENT_ID`.
- `components/global-elevenlabs-widget.tsx` now renders the stock floating widget on every non-homepage route. Keep it out of the way on `/` so the homepage hero remains the primary experience, and keep the existing `/get-started` step-aside rule so we do not show two chat launchers at once when the dedicated sales chat is present.
...
- If you change widget placement or route-aware mounting, also re-run `pnpm exec playwright test __tests__/visual/global-elevenlabs-widget.spec.ts --project=desktop-chromium`.
