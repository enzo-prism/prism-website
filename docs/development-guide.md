...
- `/ig`, `/tiktok`, and `/youtube` rely on `components/social-link-hub.tsx` for shared CTA/outbound-link tracking. Keep the header profile link and the external YouTube card on `trackExternalLinkClick`, and the internal action cards (`/websites`, `/case-studies`, `/content-os`, `/prism-infinity`, `/refer`, `/get-started`, `/wall-of-love`) on `trackCTAClick` with the `{platform} landing header|actions` locations, so inbound social traffic keeps a full click trail. The Marble App Store card was removed.
...
- Primary nav labels live in `lib/constants.ts` (`NAV_ITEMS`): a flat seven-item list — the four offers (`OFFER_NAV_ITEMS`), the proof pair wall of love + case studies (`PROOF_NAV_ITEMS`), and `CONTACT_NAV_ITEM`, with no dropdown and no CTA button. The desktop rail renders the three hairline-divided groups (offers | proof | contact); the mobile sheet renders the same flat list with mono index prefixes (01–07). `/pricing` and `/get-started` are intentionally NOT in the top nav (they stay reachable via footer, homepage callout, and offer pages). When you add or remove an item, verify the desktop rail still fits at the `lg` (1024px) takeover width, the mobile sheet, locked route snapshots, and a representative case study detail page.
