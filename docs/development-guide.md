...
- `/ig`, `/tiktok`, and `/youtube` rely on `components/social-link-hub.tsx` for shared CTA/outbound-link tracking. Keep the header profile link on `trackExternalLinkClick`, and the two internal action cards (`/websites` primary, `/refer` quiet — the only cards since the 2026-07-29 simplification) on `trackCTAClick` with the `{platform} landing header|actions` locations, so inbound social traffic keeps a full click trail. See `docs/pages-overview.md` for the hub's copy and typography rules (sans mixed-case body; mono-caps only for the wordmark and footer).
