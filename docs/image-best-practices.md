...
- Generate (or refresh) these assets with `node scripts/capture-case-study-screenshots.mjs [slug ...]`. The script uses Playwright + headless Chromium to capture 1440×900 desktop and 390×844 mobile JPEGs from the `websiteUrl` field of each entry in `lib/case-study-data.ts`. Existing files are skipped unless you pass `--force` (use `--mobile-only` to refresh just the Cover Flow / mobile frame). Mobile captures dismiss consent sheets, hide chat widgets, and scroll sparse heroes so the branded top of the page fills the card crop.
