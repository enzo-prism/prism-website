...
| `NEXT_PUBLIC_ELEVENLABS_WIDGET_DISABLED`             | Optional                                 | Public kill switch for deterministic visual builds or local debugging when you need the stock widget fully suppressed.                                                                                                                                                                       | Disabled only when explicitly set to a truthy value (`1`, `true`, `yes`, `on`). Leave unset for normal development and production.                                                                                                                 | `lib/elevenlabs.ts`, `components/elevenlabs/ElevenLabsWidget.tsx`, `components/home/HomeHeroAgent.tsx`, `components/global-elevenlabs-widget.tsx` |
...
- ElevenLabs markdown links are plain text unless the widget trusts the target host. This repo sets `markdown-link-allowed-hosts` on both widget entry points, defaults the allowlist to trusted booking hosts (`calendar.notion.so`, `cal.com`) plus Prism domains, relies on the documented `markdown-link-include-www="true"` behavior, and forces `markdown-link-allow-http="false"` so only HTTPS links remain clickable. Override `NEXT_PUBLIC_ELEVENLABS_MARKDOWN_LINK_ALLOWED_HOSTS` if the agent should link somewhere else.
...
- The homepage hero uses the stock inline widget with `variant="expanded"` and `dismissible="false"` so it stays stable as the primary above-the-fold experience without unsupported Shadow DOM overrides.
- The homepage widget host is intentionally scoped to the hero container rather than left as a viewport-wide fixed layer. If you debug future scroll/layout bugs, verify the host still computes to `position: absolute` with `inset: 0` inside `.home-hero-agent`.
...
- The floating host must keep its elevated z-index so sticky nav, skip links, charts, and other fixed page chrome never render above the widget’s visible surface.
...
- `NEXT_PUBLIC_ELEVENLABS_WIDGET_DISABLED=true` is the preferred way to keep visual tests deterministic or to isolate non-widget page debugging. For actual widget debugging, leave it unset and use a fresh `pnpm build && pnpm start -p <port>` preview so you are testing the same runtime shape production gets.
