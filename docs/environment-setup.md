...
| `NEXT_PUBLIC_WEBSITE_INTAKE_FORM_ENDPOINT`           | Optional            | Formspree endpoint for the focused `/website-intake` PRO website funnel.                 | Defaults to `https://formspree.io/f/xpqebnbz` (temporary reuse of the June 2026 website-request form). Create a dedicated **Website Intake** form, then set this in Vercel Production + Preview and update the code fallback. See [`docs/forms.md`](forms.md#formspree-dashboard-handoff-needs-account-access). | `components/forms/WebsiteIntakeForm.tsx`                                                                                |
...
- Marketing forms post to public `https://formspree.io/f/{hash}` endpoints. This repo has no Formspree MCP, CLI project, or `FORMSPREE_DEPLOY_KEY`.
- Creating or renaming forms requires a signed-in Formspree dashboard session for `enzo@design-prism.com`.
- After changing any `NEXT_PUBLIC_*` Formspree endpoint in Vercel, redeploy. Those values are inlined at build time.
- `/website-intake` still needs a dedicated Formspree form. The current `xpqebnbz` fallback is only so submissions are not blocked. Follow the dashboard handoff in [`docs/forms.md`](forms.md#formspree-dashboard-handoff-needs-account-access).
