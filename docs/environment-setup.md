...
- `components/global-elevenlabs-widget.tsx` mounts the stock floating widget on non-mobile public inner pages; `/`, mobile viewports, `/apply`, `/ig`, and `/tiktok` intentionally stay widget-free.
- On `/get-started`, this launcher stays available as a floating assistive surface on non-mobile viewports while the primary conversion path points into the built-in Free Practice Audit flow. On mobile and `/apply`, the launcher is suppressed so the active audit request stays focused.
