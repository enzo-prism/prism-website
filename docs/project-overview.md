...
- `components/navbar.tsx` and `components/footer.tsx` are the single shared chrome layer, repositioned around the offers and overhauled to be fully responsive (the nav uses an `xl` breakpoint with a full-height mobile panel that locks scroll on both `html` and `body`, marks the page behind it `inert`, and animates in; a compact "Order" CTA stays visible in the header on phones. The footer uses a responsive column grid + monochrome icon socials, 44px touch targets, and a "Refer a friend — $100" link to `/refer`). Nav labels come from `NAV_ITEMS` in `lib/constants.ts`: order (`/websites`), content os, dental os, prism infinity, pricing, get started, contact. The footer leads with two CTAs — "Order a website" (`/websites`) and "Get started free" (`/get-started`).
...
- `components/forms/WebsiteOrderForm.tsx` is the active `/websites` order flow: a launcher panel plus a fullscreen one-question-at-a-time dialog (portal; scroll lock + `inert` behind it; `#order` deep link; same-tab draft in `sessionStorage` `prism_website_order_draft_v1`; order-manifest rail), ending in a staged in-dialog success state that opens the `$300` Stripe Payment Link (new tab) via `lib/payment-links.ts`. It replaces the retired `WebsiteBuildEstimatorForm.tsx` dynamic estimator (no thank-you redirect, no price estimator). `components/forms/ReferralForm.tsx` powers the `/refer` $100 referral program.
...
- Other core marketing routes: `app/about`, `app/get-started`, `app/apply`, `app/services`, `app/ads`, `app/local-listings`, `app/seo`, `app/ai-seo-services`, `app/aeo`, `app/proof`, and `app/wall-of-love`. `app/refer` is the noindex `$100` referral program (form-first, `/referral`+`/referrals`+`/affiliate` redirect there); `app/tiktok`, `app/ig`, and `app/youtube` are the noindex social link hubs (`components/social-link-hub.tsx`).
