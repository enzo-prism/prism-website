...
- **Sales chat availability** – `/get-started` chat renders only when deterministic runtime is ready: `uiAvailable = SALES_CHAT_ENABLED && ctaUrlsConfigured` (`SALES_CHAT_BOOKING_URL`, `SALES_CHAT_WEBSITE_OVERHAUL_CHECKOUT_URL`, `SALES_CHAT_GROWTH_PARTNERSHIP_SIGNUP_URL`). Lead emission requires `SALES_CHAT_LEADS_WEBHOOK_URL` (and `SALES_CHAT_LEADS_WEBHOOK_SECRET` for non-Formspree webhooks). `AI_GATEWAY_*` is required only when AI response mode is enabled (`SALES_CHAT_AI_FALLBACK_ENABLED=true` and `SALES_CHAT_AI_RESPONSE_MODE!=off`).
