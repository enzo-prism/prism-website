...
- **Get-started assistant surface** – `/get-started` no longer mounts the custom `SalesChat` UI. On non-mobile viewports, the live page relies on the stock ElevenLabs floating widget from `components/global-elevenlabs-widget.tsx`; mobile viewports, `/apply`, and the minimal `/ig` and `/tiktok` social credit pages intentionally suppress that widget so focused flows stay clean. Do not reintroduce sales-chat runtime gating unless the product direction explicitly changes.
...
- For the public ElevenLabs launcher (`components/global-elevenlabs-widget.tsx`), prefer the stock floating widget with documented attributes and host-level styling only. It now mounts on non-mobile public inner pages but intentionally stays off the homepage, mobile viewports, `/apply`, `/ig`, and `/tiktok`; it defaults to collapsed when no explicit user preference exists and should keep a top-level z-index so site chrome never renders above it. If we need heavier visual customization later, move to ElevenLabs' official UI/SDK layer instead of styling undocumented internal widget structure.
