...
Install dependencies with `pnpm install`. Run `pnpm dev` to start the local Next.js server, `pnpm build` for a production bundle, and `pnpm start` to smoke-test the build. Quality gates include `pnpm lint`, `pnpm typecheck`, and `pnpm test`; combine them before pushing with `pnpm lint && pnpm typecheck && pnpm test`. For pricing-sensitive changes, run `pnpm verify:pricing-consistency`. For `/get-started` chatbot changes, also run `pnpm test:sales-chat:core`, `pnpm test:sales-chat:e2e`, and `pnpm test:sales-chat:stress` (or set `SALES_CHAT_STRESS_RUNS` for shorter local loops).
