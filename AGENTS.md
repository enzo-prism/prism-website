...
- **Assistant surface** – `/get-started` no longer mounts the custom `SalesChat` UI or the stock ElevenLabs launcher. The public ElevenLabs floating widget from `components/global-elevenlabs-widget.tsx` is intentionally limited to non-mobile `/pricing` and `/contact` only; mobile viewports and every other public route should stay widget-free so focused flows stay clean. Do not reintroduce sales-chat runtime gating unless the product direction explicitly changes.
- **Canonical pricing policy** – `/pricing` is the canonical Growth Dashboard pricing URL: free Growth Dashboard, included Light Audit, normally `$500` Deep Growth Audit, 60-day Growth Sprints starting at `$3,500`, and optional ongoing Growth Partner support starting at `$1,500/month`. `/websites` is the dedicated exception for the selective one-time website build offer starting at `$300`, and `/founder-os` is the premium Founder OS exception (its own Blueprint/installation/managed ladder on a distinct light surface); do not fold either offer into the main pricing model. Legacy fixed-plan routes should redirect to `/pricing`.
...
- For the public ElevenLabs launcher (`components/global-elevenlabs-widget.tsx`), prefer the stock floating widget with documented attributes and host-level styling only. It now mounts only on non-mobile `/pricing` and `/contact`; it defaults to collapsed when no explicit user preference exists and should keep a top-level z-index so site chrome never renders above it. If we need heavier visual customization later, move to ElevenLabs' official UI/SDK layer instead of styling undocumented internal widget structure.
