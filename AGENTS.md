...
- **Sales chat availability** – `/get-started` chat renders only when deterministic runtime is ready: `uiAvailable = SALES_CHAT_ENABLED && ctaUrlsConfigured && stateSigningConfigured`. CTA URLs are `SALES_CHAT_BOOKING_URL`, `SALES_CHAT_WEBSITE_OVERHAUL_CHECKOUT_URL`, and `SALES_CHAT_GROWTH_PARTNERSHIP_SIGNUP_URL`. State signing is configured by setting `SALES_CHAT_STATE_SECRET` explicitly (preferred) or by falling back to another server secret (`SALES_CHAT_LEADS_WEBHOOK_SECRET`, `SALES_CHAT_EVENTS_WEBHOOK_SECRET`, `AI_GATEWAY_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, or `RESEND_API_KEY`). Lead emission still requires `SALES_CHAT_LEADS_WEBHOOK_URL` (and `SALES_CHAT_LEADS_WEBHOOK_SECRET` for non-Formspree webhooks), but missing lead-dispatch config should not prevent the chat from mounting. `AI_GATEWAY_*` is required only when AI response mode is enabled (`SALES_CHAT_AI_FALLBACK_ENABLED=true` and `SALES_CHAT_AI_RESPONSE_MODE!=off`).
...
- **Deploy mode policy** – CI deploys production with source mode (`npx vercel deploy --prod --yes`) after `pnpm test:visual:locked` and `pnpm test:sales-chat:e2e`; do not rely on `vercel deploy --prebuilt` as the default production path for this repo.
- **Sales chat API contract** – Treat `app/api/chat/route.ts` as deterministic v2 JSON only (`sessionId`, `sourcePage`, `inputType`, `inputValue`, optional `buttonId`, optional `stateToken`, optional `conversationState`). `stateToken` is the authoritative signed conversation state and should be round-tripped on every follow-up turn; raw `conversationState` is compatibility/debug input only. Keep `x-sales-chat-route` tracing headers intact and document any contract change in `docs/development-guide.md` and `docs/environment-setup.md`.
...
- For the homepage ElevenLabs hero (`components/home/HomeHeroAgent.tsx`), default to the stock ElevenLabs widget via `components/elevenlabs/ElevenLabsWidget.tsx` and preserve the official widget design as closely as possible. The only approved Shadow DOM customization is the homepage-only desktop centering override that keeps the expanded card visually centered in the hero.
- Keep the homepage hero roomy enough for the stock widget to breathe (`HomeHeroAgent` currently reserves roughly `31rem`–`33rem` of height depending on breakpoint) and prefer wrapper spacing, width, and hero staging changes over internal widget styling.
- All non-homepage pages should keep using the stock floating widget from `components/global-elevenlabs-widget.tsx`. `components/elevenlabs/PrismElevenLabsPanel.tsx` and the richer client-tool helpers in `lib/elevenlabs.ts` are archival/R&D paths right now, not the live production implementation.
- In local dev, the ElevenLabs widget may log `[ConversationalAI] Cannot fetch config ... signal is aborted without reason` during remount/cleanup. Treat that as benign widget cleanup noise if the widget still renders and the production build is clean.
