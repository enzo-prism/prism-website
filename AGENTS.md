...
- **Get-started assistant surface** – `/get-started` no longer mounts the custom `SalesChat` UI. The live page relies on the stock ElevenLabs floating widget from `components/global-elevenlabs-widget.tsx`; `/apply` intentionally suppresses that widget so the active application flow stays focused. Do not reintroduce sales-chat runtime gating unless the product direction explicitly changes.
