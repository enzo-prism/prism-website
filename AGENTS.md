...
- **Sales chat availability** – `/get-started` chat renders only when deterministic runtime is ready: `uiAvailable = SALES_CHAT_ENABLED && ctaUrlsConfigured` (`SALES_CHAT_BOOKING_URL`, `SALES_CHAT_WEBSITE_OVERHAUL_CHECKOUT_URL`, `SALES_CHAT_GROWTH_PARTNERSHIP_SIGNUP_URL`). `SALES_CHAT_LEADS_WEBHOOK_*` must also be configured for lead emission, and `AI_GATEWAY_*` is required only when `SALES_CHAT_AI_FALLBACK_ENABLED=true`.
